...
def protected_secret(password, secret, num_attempts):
    ...
    def get_secret(password_attempt):
        "*** YOUR CODE HERE ***"
        def get_secret2(left_attempts):
            def get_secret3(passwd_attempt):
                left = left_attempts
                if left <= 0:
                    print("SECRET LOCKED")
                elif passwd_attempt == password:
                    print(secret)
                else:
                    print("INCORRECT PASSWORD")
                    left -= 1
                return get_secret2(left)
            return get_secret3
        return get_secret2(num_attempts)(password_attempt)
    return get_secret
